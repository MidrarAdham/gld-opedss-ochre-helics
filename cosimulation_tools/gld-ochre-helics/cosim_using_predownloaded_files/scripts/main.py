...
if __name__ == '__main__':
    ...
    pf.fig1_raw_binary_states(
        wh_df=list(wh_loader.all_dfs.values())[0],
        hvac_df=list(hvac_loader.all_dfs.values())[0]
    )
    # pf.fig2_posterior_mean_comparison(
    #     histories_low=wh_histories_low,    # discount=0.01
    #     histories_high=wh_histories_high,  # discount=0.3
    # )
    pf.fig3_posterior_variance(wh_histories, hvac_histories)
    pf.fig4_state_matrix_heatmap(results['wh_state_matrix'], results['hvac_state_matrix'])

    # Section 2
    pf.fig5_mean_matrix_heatmap(results['wh_mean_matrix'], results['hvac_mean_matrix'])
    pf.fig6_aggregated_probability_vectors(
        x_wh=results['x_wh'],
        x_hvac=results['x_hvac']
    )
    ...
    pf.fig10_delta_ols_predicted_vs_truth(
        wh_predicted=results['delta_wh_predicted'],
        hvac_predicted=results['delta_hvac_predicted'],
        wh_ground_truth=wh_ground_truth['power_out'],
        hvac_ground_truth=hvac_ground_truth['power_out']
    )
